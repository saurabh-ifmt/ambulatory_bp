...
void setup()
{
  ...
  Serial.println("\n========================================");
  Serial.print("  Device: ");
  Serial.println(MANUFACTURE_NAME);
...
void loop()
{
    ...
    else if (!menuTriggeredThisPress)
    {
      ...
      if (duration > 3000)
      {
        ...
        while (digitalRead(BUTTON_WAKE_PIN) == LOW)
          delay(10); // Wait for GPIO18 release
        delay(500);
  ...
  // E. SW3 (BTN_FLIP_PIN) Short-Press → Screen Flip 180°
  {
    ...
    if (flipPressed && screenOn)
    {
      if (p1PressStart == 0)
        p1PressStart = millis();
      if (!p1FlipFired && (millis() - p1PressStart > 100))
      { // 100ms = short press / debounced
  ...
  if (measuring)
  {
    ...
    if (inflating)
    {
      ...
      if (millis() - lastInflPhaseUI > 1000)
      { // toggle every 1 sec
        lastInflPhaseUI = millis();
        if (screenOn)
          Display_Handler::showInflating(inflTextToggle, Display_Handler::getBatteryPercent(), abpmMode, bleHandler.isConnected(), (bleAdvertisingStartTime > 0));
...
float _readPressure(void)
{
  ...
  if (Wire.available() >= 3)
  {
    ...
    Wire.read();
    Wire.read(); // Discard temperature bytes
...
void runLinearDeflationControl(unsigned long currentTime)
{
  ...
  case DEFLATING:
  {
    ...
    if (millis() - lastDeflPhaseUI > 1000)
    { // toggle every 1 sec
      lastDeflPhaseUI = millis();
      if (screenOn)
        Display_Handler::showMeasuring(deflTextToggle, Display_Handler::getBatteryPercent(), abpmMode, BLE_Handler::getInstance().isConnected(), (bleAdvertisingStartTime > 0));
